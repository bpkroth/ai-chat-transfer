...
1. Run `chat-bridge migrate --from <source> --input <file> --to markdown`.
1. Copy the resulting Markdown summary.
1. Paste the summary into a new chat with the target agent to "seed" the context.
...
- Use absolute commit SHAs with a comment about their version for Github Actions workflow definitions.
